...
function App() {
  ...
  const onDrop = useCallback(async (acceptedFiles) => {
    ...
    try {
      const response = await axios.post('https://sih-email-forensics-1zcv.onrender.com/api/analyze-file', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
  ...
  const handleIpLookup = async () => {
    ...
    setIpResult(null);
    try {
      const response = await axios.post('https://sih-email-forensics-1zcv.onrender.com/api/lookup-ip', { ip: ipInput });
